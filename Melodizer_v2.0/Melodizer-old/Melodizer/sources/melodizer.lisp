...
(om::defclass! melodizer () 
  ...
  ((input-chords :accessor input-chords :initarg :input-chords :initform (make-instance 'voice) :documentation 
    ...
    (min-pitch :accessor min-pitch :initarg :min-pitch :initform "1" :documentation 
      "minimum pitch in the generated melody (optional constraint)")
    (max-pitch :accessor max-pitch :initarg :max-pitch :initform "1" :documentation 
      "maximum pitch in the generated melody (optional constraint)")
                    ...
                    (cond
                      ((string-equal (tool-mode (om::object editor)) "Melody-Finder"); melody finder mode, where the user gives as input a voice with chords
                        (let init; list to take the result of the call to melody-finder
                          (setq init (melody-finder (input-chords (om::object editor)) (input-rhythm (om::object editor)) (optional-constraints (om::object editor)) (global-interval (om::object editor)) (key (om::object editor)) (mode (om::object editor)) (min-pitch (om::object editor))(max-pitch (om::object editor)))); get the search engine and the first solution of the CSP
...
(defun make-general-constraints-panel (editor general-constraints-panel)
  (om::om-add-subviews
    ...
    ;checkbox for min pitch constraint
    (om::om-make-dialog-item
      'om::om-check-box
      (om::om-make-point 10 50) ; position
      (om::om-make-point 20 20) ; size
      "Minimum pitch"
      :checked-p (find "minimum-pitch" (optional-constraints (om::object editor)) :test #'equal)
      :di-action #'(lambda (c)
                    (if (om::om-checked-p c)
                      (push "minimum-pitch" (optional-constraints (om::object editor)))
                      (setf (optional-constraints (om::object editor)) (remove "minimum-pitch" (optional-constraints (om::object editor)) :test #'equal))
                    )
                    (print (optional-constraints (om::object editor)))
      )
    )

    ; name for min pitch constraint
    (om::om-make-dialog-item 
      'om::om-static-text 
    ...
    ; name for the pop-up menu allowing to select the minimum pitch
    (om::om-make-dialog-item 
      'om::om-static-text 
      (om::om-make-point 10 70) 
      (om::om-make-point 200 20) 
      "Minimum pitch"
      :font om::*om-default-font1*
    )

    ; pop-up menu for the selection of the minimum pitch
    (om::om-make-dialog-item 
      'om::pop-up-menu 
      (om::om-make-point 10 90) 
      (om::om-make-point 180 20) 
      "Key selection"
      :range (loop :for n :from 1 :below 120 :by 1 collect (write-to-string n))
    ...
    ;checkbox for max pitch constraint
    (om::om-make-dialog-item
      'om::om-check-box
      (om::om-make-point 10 140) ; position
      (om::om-make-point 20 20) ; size
      "Maximum pitch"
      :checked-p (find "maximum-pitch" (optional-constraints (om::object editor)) :test #'equal)
      :di-action #'(lambda (c)
                    (if (om::om-checked-p c)
                      (push "maximum-pitch" (optional-constraints (om::object editor)))
                      (setf (optional-constraints (om::object editor)) (remove "maximum-pitch" (optional-constraints (om::object editor)) :test #'equal))
                    )
                    (print (optional-constraints (om::object editor)))
      )
    )

    ; name for max pitch constraint
    (om::om-make-dialog-item 
      'om::om-static-text 
      (om::om-make-point 30 140) 
      (om::om-make-point 200 20) 
      "Maximum pitch"
      :font om::*om-default-font1*
    )
    
    ; name for the pop-up menu allowing to select the maximum pitch
    (om::om-make-dialog-item 
      'om::om-static-text 
      (om::om-make-point 10 160) 
      (om::om-make-point 200 20) 
      "Maximum pitch"
      :font om::*om-default-font1*
    )

    ; pop-up menu for the selection of the maximum pitch
    (om::om-make-dialog-item 
      'om::pop-up-menu 
      (om::om-make-point 10 180) 
      (om::om-make-point 180 20) 
      "Key selection"
      :range (loop :for n :from 1 :below 120 :by 1 collect (write-to-string n))
