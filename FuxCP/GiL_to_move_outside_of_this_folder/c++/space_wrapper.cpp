...
void WSpace::constrain(const Space& _b) {
    ...
    ofstream myfile;
    myfile.open ("/home/tibo/Documents/TFE/TFE-Composition-Musicale-main/TFE-Melodizer-GiL/Melodizer/tests/cost.txt", fstream::app);
    myfile << "percent_diff: ";
    myfile << b.percent_diff << endl;
    myfile.close();
}
...
void WSpace::branch(int n, int* vids, int var_strategy, int val_strategy) {
    ...
    else if(var_strategy == 4){//INT_VAR_MIN_MIN()
        var_strat = INT_VAR_MIN_MIN();
    }
    else if(var_strategy == 5){//INT_VAR_MIN_MAX()
        var_strat = INT_VAR_MIN_MAX();
    }
    else if(var_strategy == 6){//INT_VAR_DEGREE_SIZE_MAX()
        var_strat = INT_VAR_DEGREE_SIZE_MAX();
    }
    else if(var_strategy == 7){//INT_VAR_REGRET_MIN_MIN()
        var_strat = INT_VAR_REGRET_MIN_MIN();
    }
    else if(var_strategy == 8){//INT_VAR_REGRET_MIN_MAX()
        var_strat = INT_VAR_REGRET_MIN_MAX();
    }
    ...
    else if(val_strategy == 5){//INT_VAL_RANGE_MIN()
        val_strat = INT_VAL_RANGE_MIN();
    }
 ...
 */
void WSpace::set_cost(int vid) {
    cost_id = vid;
    the_cost = int_vars.at(vid);
}
...
WSpace::WSpace(WSpace& s): IntMinimizeSpace(s), int_vars(s.i_size), bool_vars(s.b_size), set_vars(s.s_size), i_size(s.i_size), b_size(s.b_size), s_size(s.s_size), cost_id(s.cost_id), the_cost(s.the_cost),
                           var_sol_size(s.var_sol_size), solution_variable_indexes(s.solution_variable_indexes), percent_diff(s.percent_diff) {
    // ofstream myfile;
    // myfile.open ("/home/tibo/Documents/TFE/TFE-Composition-Musicale-main/TFE-Melodizer-GiL/Melodizer/tests/cost.txt", fstream::app);
    // myfile << "in_space: ";
    // myfile << the_cost << endl;
    // myfile.close();
