...
(defparameter gil::INT_VAR_MIN_MIN 4) ;select the variable with the smallest minimum value
(defparameter gil::INT_VAR_MIN_MAX 5) ;select the variable with the largest minimum value
(defparameter gil::INT_VAR_DEGREE_SIZE_MAX 6) ;select the variable with the highest degree divided by domain size
(defparameter gil::INT_VAR_REGRET_MIN_MIN 7) ; select the variable with the smallest minimum regret
(defparameter gil::INT_VAR_REGRET_MIN_MAX 8) ; select the variable with the largest minimum regret
...
(defparameter gil::INT_VAL_MED 4) ; select the greatest value not bigger than the median
(defparameter gil::INT_VAL_RANGE_MIN 5) ; select the values from smallest range, if domain has several ranges; otherwise, values not greater than mean
